...
# server.main is imported below while this module is being initialized, so the
...
class OverlayWindow(QMainWindow):
    def __init__(self, cfg, screen_w, screen_h):
        ...
        # Redirecting MapGenie's main bundle through a custom Qt scheme can
        # leave the Mapbox canvas black on current MapGenie builds.  The
        # companion overlay works without this optional category hook, so keep
        # it opt-in until the remote bundle needs/has a verified patch.
        if os.environ.get('CD_PATCH_MAPGENIE_MAP_JS', '0') == '1':
    ...
    def _on_load_finished(self, ok):
        if ok:
            ...
            QTimer.singleShot(5000, self._log_webgl_diagnostics)
            if self._native_realtime_enabled:
                self._native_realtime_timer.start()
            else:
                self._native_realtime_timer.stop()

    def _log_webgl_diagnostics(self):
        script = r'''(() => {
          const canvases = Array.from(document.querySelectorAll('canvas'));
          const details = canvases.map((c, i) => {
            let gl = null, error = null, renderer = null;
            try {
            ...
            return {i, width:c.width, height:c.height, webgl:!!gl, renderer, error};
          });
          const resources = performance.getEntriesByType('resource').map(x => x.name)
            .filter(x => /tile|map|webp|png|jpg/i.test(x)).slice(-30);
          return JSON.stringify({url:location.href, canvases:details, resources});
        })()'''
        self._view.page().runJavaScript(script, self._write_webgl_diagnostics)

    def _write_webgl_diagnostics(self, result):
        try:
            path = os.path.join(_APP_DIR, 'cd_webengine.log')
            with open(path, 'w', encoding='utf-8') as f:
                f.write(str(result or 'No WebGL diagnostic result'))
        except Exception:
            pass
...
def _start_server_thread(ready_event):
    ...
    try:
        ...
        # Windowed PyInstaller builds have no stderr. Always leave a standalone
        # traceback next to the executable so startup failures are actionable.
        try:
            with open(os.environ.get('CD_LOG_FILE', 'cd_server.log'),
                      'a', encoding='utf-8') as log_file:
                log_file.write('\nSERVER STARTUP TRACEBACK\n')
...
def main():
    ...
    # MapGenie's current MapLibre renderer is incompatible with the old
    ...
    if overlay_mode == 'full':
        ...
        # The Full-mode UI uses an inline Nearby panel, so it is safe to keep
    ...
    # QtWebEngine's Chromium is old enough to block some modern GPU/driver
    # combinations. MapGenie then creates its canvas but renders it black.
    sys.argv += [
...
def _run_server_only(tray_title='CD Companion — Server Only'):
    ...
    _tray, _tray_parent = _create_tray_icon(app, tray_title)
