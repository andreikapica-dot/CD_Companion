...
class CaveBuilderMixin:
    ...
    self.OFF_CAM_YAW, self.OFF_PHYS_POS, self.BLOCK_SZ, self.ORIG_HOOK_A, self.ORIG_HOOK_B,
    ...
    def _build_cave_e(self):
        ...
        if self._hook_e_predecessor and self._hook_e_predecessor_patch_size == 5:
            return self._build_cave_e_pred5(tp)
        ...
        c += b'\x4C\x89\x68\x60'                       # mov [rax+0x60], r13  (save position pointer)
        c += b'\x41\x0F\x10\x45\x00'                    # movups xmm0, [r13]
        c += b'\x0F\x11\x40\x40'                        # movups [rax+0x40], xmm0
        c += b'\x0F\x28\xC6'                             # movaps xmm0, xmm6 (restore original)
        ...
        c += b'\x0F\x10\x48\x40'                       # movups xmm1, [rax+40]  current physics
    ...
    def _build_cave_e_pred5(self, tp):
        ...
        c += b'\x48\x89\x58\x28'                       # mov [rax+0x28], rbx  (save physics entity)
        c += b'\x4C\x89\x68\x60'                       # mov [rax+0x60], r13  (save position pointer)
        c += b'\x0F\x11\x40\x50'                        # movups [rax+0x50], xmm0
        c += b'\x41\x0F\x10\x45\x00'                    # movups xmm0, [r13]
        c += b'\x0F\x11\x40\x40'                        # movups [rax+0x40], xmm0
        c += b'\x0F\x10\x40\x50'                        # movups xmm0, [rax+0x50]
