...
INVULN_SECONDS = 10
WS_PORT        = 7891
WS_HOST        = '127.0.0.1'  # local Full window and browser extension
...
from server.persistence import (
    CALIBRATION_FILES,
    _load_calibration, _load_saved_calibration, _save_calibration, _get_cal, _cal_cache,
...
_crosshair_target: tuple = None # browser-map target used by the F5 hotkey
...
async def _handle_client(websocket):
    global _waypoints, _pre_teleport_pos, _crosshair_target, _default_teleport_y
    ...
    try:
        async for raw in websocket:
            ...
            elif action == "set_hotkeys":
                # Full WebView2 mode has its own small settings panel.  Keep
                # validation here, then let the polling thread reload it.
                requested = cmd.get("hotkeys")
                saved = _load_hotkey_settings()
                ok, err = isinstance(requested, dict), "Invalid hotkey data"
                if ok:
                    for hotkey_id, raw in requested.items():
                        if hotkey_id not in DEFAULT_HOTKEYS or not isinstance(raw, dict):
                            continue
                        try:
                            vk = int(raw.get("vk", 0))
                            mods = [int(m) for m in (raw.get("mods") or [])]
                        except (TypeError, ValueError):
                            ok, err = False, "Invalid hotkey"
                            break
                        if not 1 <= vk <= 0xFE or any(m not in HOTKEY_MODIFIERS for m in mods):
                            ok, err = False, "Unsupported hotkey"
                            break
                        saved[hotkey_id] = {
                            "vk": vk,
                    ...
                    if ok:
                        _save_hotkey_settings(saved)
                        log.info("Hotkeys updated from Full mode")
                await _safe_send(websocket, json.dumps({
                    "type": "hotkeys_saved", "ok": ok, "err": err if not ok else ""
                }))

            elif action == "hotkey_editing":
                # Do not trigger an existing global binding while its field is
                # being recorded in the Full-mode settings panel.
                set_keyboard_hotkey_paused(bool(cmd.get("active")))
            ...
            elif action == "set_crosshair_target":
                lng, lat, y = cmd.get("lng"), cmd.get("lat"), cmd.get("y")
                realm = cmd.get("realm") or (_last_pos["realm"] if _last_pos else "pywel")
                ok, err = False, "Dados inválidos para alvo F5"
                ax = ay = az = None
                if None not in (lng, lat, y) and realm in CALIBRATION_FILES:
                    game_pos = lnglat_to_game(float(lng), float(lat), _get_cal(realm))
                    if game_pos:
                        ax, az = game_pos
                        ay = float(y)
                        _crosshair_target = (ax, ay, az, realm)
                        ok, err = True, ""
                        log.info("F5 crosshair target saved: (%.1f, %.1f, %.1f) realm=%s", ax, ay, az, realm)
                    else:
                        err = "Falha ao converter coordenadas do mapa"
                await _safe_send(websocket, json.dumps({
                    "type": "crosshair_target_result", "ok": ok, "err": err,
                    "x": ax, "y": ay, "z": az,
                }))
...
def _hotkey_thread():
    ...
    while True:
        ...
        # Settings can be edited from the Full-mode browser panel while this
        # thread is already running.  Reloading once per second is negligible
        # and makes the new keyboard bindings effective without a restart.
        if time.monotonic() >= next_hotkey_reload:
            next_hotkey_reload = time.monotonic() + 1.0
            refreshed = _load_hotkey_settings()
            if refreshed != hotkeys:
                hotkeys = refreshed
                key_state.clear()
...
async def _hotkey_teleport_marker():
    """Teleports to the saved browser target, with game-marker fallback."""
    global _pre_teleport_pos
    if not _engine or not _engine.hooks_installed:
        return
    if _crosshair_target:
        ax, ay, az, realm = _crosshair_target
        log.info("F5 teleport uses browser crosshair target (realm=%s)", realm)
    else:
        dest = _engine.get_map_dest()
        if not dest:
            log.info("F5 teleport: no browser target or game marker")
            return
        ax = dest[0]
        ay = _effective_marker_y(dest[1]) + HEIGHT_BOOST
        az = dest[2]
...
async def _main(ready_event=None):
    ...
    async with contextlib.AsyncExitStack() as stack:
        log.info("Starting WS bind on %s:%d", WS_HOST, WS_PORT)
        await stack.enter_async_context(
            websockets.serve(_handle_client, WS_HOST, WS_PORT, origins=None))
        log.info("WS  → ws://%s:%d  (Chrome / overlay)", WS_HOST, WS_PORT)
